...
%%
ROI = {'IPS', 'SPL', 'V3A', 'hMT', 'FEF', 'insula'};
...
for run = 1:4
    ...
    for trial = 1:4
        ...
        pattcomp(volumes.(runs{run}).PattComp(trial,1):volumes.(runs{run}).CompPatt(trial,1)+nvolsmov) = 1;
    end
...
for run = 1:4
    ...
    task_effect.CompPatt.(runs{run}) = CompPatt_effect;
    task_effect.PattComp.(runs{run}) = PattComp_effect;
    task_effect.Discard.(runs{run}) = Discard_effect;
    task_effect.Static.(runs{run}) = Static_effect;
    task_effect.MAE.(runs{run}) = MAE_effect;
end
