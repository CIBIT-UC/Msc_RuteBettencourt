...
datacon = load('correlationTCs_wdSz10.mat'); % Correlation time courses (windowed)
...
for cc = 1:nCombinations
    ...
    for ee = 1:nEffects
        ...
        corrPerEffect.(effects{ee}).(ROIs_clean{comb(cc,1)}).(ROIs_clean{comb(cc,2)}).CompPatt.Spearman = zeros(nRunsPerEffect_CompPatt(ee),21); 
        corrPerEffect.(effects{ee}).(ROIs_clean{comb(cc,1)}).(ROIs_clean{comb(cc,2)}).PattComp.Spearman = zeros(nRunsPerEffect_PattComp(ee),21); 
...
%                     datacon.correlationTCs.(ROIs_clean{comb(cc,1)}).(ROIs_clean{comb(cc,2)}).corrPearson_PattComp.(subjects{ss})(corrIdxBlock_PattComp:corrIdxBlock_PattComp+9,rr);
                ...
                corrPerEffect.(effects{auxEffect_CompPatt}).(ROIs_clean{comb(cc,1)}).(ROIs_clean{comb(cc,2)}).CompPatt.Spearman(counterPerEffect_CompPatt(auxEffect_CompPatt), 11:15) = ...
...
for cc = 1:nCombinations
    ...
    meanCorrVols_Spearman = zeros(4,21);
    semCorrVols_Spearman = zeros(4,21);%lines effects, column correlations TCs
...
%        e1 = errorbar([1], meanCorrPre(1,xx(ee)), semCorrPre(1,xx(ee)),'color',clrMap(6+3*ee,:),'linestyle','none','markersize',20,'marker','.');
 ...
 %       e3 = errorbar([21], meanCorrPos(1,xx(ee)), semCorrPos(1,xx(ee)),'color',clrMap(6+3*ee,:),'linestyle','none','markersize',20,'marker','.');
        e4 = errorbar(1:21, meanCorrVols_Spearman(ee,:), semCorrVols_Spearman(ee,:), 'color',clrMap(6+3*ee,:),'linestyle','-','linewidth', 2,'markersize',1,'marker','.');
    ...
    xlabel('Sliding window', 'FontSize', 16); xlim([0 22]);
    xticks([1 11 16]); xticklabels({'Before effect block', 'Effect block', 'After effect block'});
...
xx = 1:3; %four effect
saveFig3Path = fullfile('/DATAPOOL', 'VPHYSTERESIS', 'ROI_SELECTION_FULL_RUN', 'WdSz10', 'SPEARMAN');
...
%     semCorrPos = zeros(3,4);
    meanCorrVols_Spearman = zeros(12,21);
    semCorrVols_Spearman = zeros(12,21);%lines effects (1st 4 group, 5-8 SS hMT cluster, 9-12 spherical, column correlations TCs
    deltaCorrVols_Spearman = zeros(3,21);
...
%             semCorrPos(idx,ee) = std(corrPerEffect.(effects{ee}).(ROIs_clean{rr}).(ROIs_clean{mt}).Spearman(:,21), 'omitnan') / sqrt(nRunsPerEffect_CompPatt(ee)+nRunsPerEffect_PattComp(ee));
        ...
        for ee = xx 
            ...
            line([1 1], [-1.1 1.1], 'linestyle', ':', 'color', 'k'); %x = 1
            line([11 11], [-1.1 1.1], 'linestyle', ':', 'color', 'k'); %x = 6
            line([16 16], [-1.1 1.1], 'linestyle', ':', 'color', 'k'); %x = 11
            line([0 22],[0 0],'linestyle',':','color','k') %y=0
...
%             e3 = errorbar([21], meanCorrPos(idx,xx(ee)), semCorrPos(idx,xx(ee)),'color',clrMap(6+3*ee,:),'linestyle','none','markersize',20,'marker','.');
        ...
        hold off
        %Subplot properties
        xlabel('Sliding window (N=10)', 'FontSize', 16); xlim([0 22]);
        xticks([1 11 16]); xticklabels({'Before effect block', 'Effect block', 'After effect block'});
        ylabel('Spearman correlation', 'FontSize', 16); 
...
%     %Figure properties
     ...
     saveas(fig3, fullfile(saveFig3Path,sprintf('WdSz10 %s.png',ROIs_clean{rr})))     
%     
%     figX = figure();
%     
%     hold on
